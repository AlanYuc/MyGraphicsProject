...
CubeRobot::CubeRobot(Mesh* cube) {
	...
	//这里以及后面的Translation的改变值都是相对于body本身的，
	// 所以body的位置移动时所有子节点一起移动
	head->SetTransform(Matrix4::Translation(Vector3(0, 30, 0)));
	body->AddChild(head);

	leftArm = new SceneNode(cube, Vector4(0, 0, 0.5f, 1));
	...
	SceneNode* leftLeg = new SceneNode(cube, Vector4(0, 0, 0.5f,1));
