...
bool Mesh::GetVertexIndicesForTri(unsigned int i, 
	...
	if (i >= triCount) {
		//三角形mesh也是用int索引来表示，索引超了就返回false
		return false;
	}
	if (numIndices > 0) {
		//有索引，那么所有的索引都是在连续的内存空间内
		a = indices[i * 3];
		b = indices[i * 3 + 1];
		c = indices[i * 3 + 2];
	}
...
void Mesh::GenerateNormals() {
	...
	for (int i = 0; i < triCount; i++) {
		...
		unsigned int b = 0;
		unsigned int c = 0;
		GetVertexIndicesForTri(i, a, b, c);//结束后，abc分别是i三角形的三个顶点

		//计算法线，向量ba 和 向量 ca进行叉乘
		Vector3 normal = Vector3::Cross((vertices[b] - vertices[a]), (vertices[c] - vertices[a]));

		//每个顶点的法线向量都加起来，方便后面做插值
		//一个顶点可能涉及多个面的法线，但最终一个顶点只会保留一个法线向量的和
		normals[a] += normal;
		normals[b] += normal;
		normals[c] += normal;
	...
	for (GLuint i = 0; i < numVertices; i++) {
		normals[i].Normalise();//每个顶点的法向量标准化
	}
}
...
void Mesh::GenerateTangents() {
	...
	for (GLuint i = 0; i < numVertices; i++) {
		//tangents的结果有可能是非规范的，需要重新做一次归一化
		//这里就是把w分量单独拎出来归一化，再把其余分量整体归一化
